 ...
 * A tag is considered unique by comparing using {@code Tag#isSameTag(Tag)}. As such, adding and updating of tags uses
 * Tag#isSameTag(Tag) for equality so as to ensure that the tag being added or updated is unique
 * in terms of identity in the UniqueTagList. However, the removal of a tag uses Tag#equals(Object)
 * so as to ensure that the tag with exactly the same fields will be removed.
 *
